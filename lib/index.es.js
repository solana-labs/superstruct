  ...
  constructor(failure, failures) {
    ...
    const {
      message
          ...
          rest = _objectWithoutPropertiesLoose(failure, ["message"]);
...
function deprecated(struct, log) {
  return new Struct(_extends({}, struct, {
...
function omit(struct, keys) {
  ...
  const subschema = _extends({}, schema);
...
function partial(struct) {
  const schema = struct instanceof Struct ? _extends({}, struct.schema) : _extends({}, struct);
...
function union(Structs) {
  ...
  return new Struct({
    ...
    coercer(value, ctx) {
      for (const S of Structs) {
        const [...tuples] = run(value, S, _extends({
          coerce: true
        }, ctx));
        const [first] = tuples;
