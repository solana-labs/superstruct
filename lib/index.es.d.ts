...
type ObjectType<S extends ObjectSchema> = Simplify<Optionalize<{
    [K in keyof S as Infer<S[K]>]: ;
}>>;
/**
 * Omit properties from a type that extend from a specific type.
 */
type OmitBy<T, V> = Omit<T, {
    [K in keyof T as V extends Extract<T[K], V> ? K : never]: ;
}[keyof T]>;
/**
...
type PartialObjectSchema<S extends ObjectSchema> = {
    [K in keyof S as Struct<Infer<S[K]> | undefined>]: ;
};
/**
...
type StructSchema<T> = [
    ...
    [K in keyof T as Describe<T[K]>]: ;
} : null;
/**
 * A schema for tuple structs.
 */
type TupleSchema<T> = {
    [K in keyof T as Struct<T[K]>]: ;
};
/**
